...
route.post('/create',[body('name').isLength({ max:30 ,min:3 }).withMessage('name should be maximum of length 30 and minimum of 3'),body('classId').isLength({ min:4,max:6 }).withMessage('minimum length of RoomId should be 4-6'),body('classcode').isLength({ min:6 }).withMessage('minimum length of code should be 6')],fetchUser,classroom_controller.createClassroom )
