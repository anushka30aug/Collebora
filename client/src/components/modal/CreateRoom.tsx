...
const CreateRoom = (): React.JSX.Element => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    dispatch(createRoom({ code: code, name: name, classId: classId })).then(
      (result) => {
        if (result.payload.success) {
          if (isActive && isAdmin) {
            // dispatch(addRoom(result.payload.data))
            dispatch(fetchClassroomsAsAdmin(true));
          }
        }
  ...
  return (
    <div>
      <Dialog
        open={open}
        onClose={() => {
          return dispatch(editShowCreateModal());
        }}
      >
        ...
        <DialogContent>
          <form onSubmit={handleSubmit}>
            ...
            <TextField
              ...
              title="(secret code should not be shared with anyone)"
              margin="dense"
              value={code}
              onChange={handleChange}
